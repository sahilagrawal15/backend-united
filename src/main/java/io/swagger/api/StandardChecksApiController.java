...
@RestController
public class StandardChecksApiController implements StandardChecksApi {

    private static final Logger log = LoggerFactory.getLogger(StandardChecksApiController.class);
    ...
    public StandardChecksApiController(ObjectMapper objectMapper, HttpServletRequest request) {
        this.objectMapper = objectMapper;
        this.request = request;
    }

    public ResponseEntity<List<Check>> standardChecksGet() {
    ...
    public ResponseEntity<Check> standardChecksPost(@Parameter(in = ParameterIn.DEFAULT, description = "", required=true, schema=@Schema()) @Valid @RequestBody NewCheck body
) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
